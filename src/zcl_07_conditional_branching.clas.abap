CLASS zcl_07_conditional_branching DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_07_conditional_branching IMPLEMENTATION.
...
ELSE. out->write( `The value of C_NUMBER is less than zero` ).
ENDIF.
...
WHEN 0.
out->write( `The value of C_NUMBER equals zero` ).
WHEN 1. out->write( `The value of C_NUMBER equals one` ).
WHEN 2. out->write( `The value of C_NUMBER equals two` ).
WHEN OTHERS. out->write( `The value of C_NUMBER equals non of the above` ).
ENDCASE.
