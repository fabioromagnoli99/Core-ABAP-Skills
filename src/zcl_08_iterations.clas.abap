CLASS zcl_08_iterations DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_08_iterations IMPLEMENTATION.
...
out->write( `----------------------------------` ).
DO c_number TIMES. out->write( `Hello World` ).
ENDDO.
...
DO.
out->write( |{ sy-index }: Value of number: { number }| ). number = number - 1.
"abort condition
IF number <= c_number.
EXIT.
ENDIF.
